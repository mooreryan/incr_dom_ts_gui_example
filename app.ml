...
module Model = struct
  type edit = { column : string; value : string } [@@deriving compare, sexp]

  type edit_state = Not_editing | Editing of edit list
...
let remember_edit (m : Model.t) (new_edit : Model.edit) =
  ...
  | Editing edits ->
      let edits =
        List.filter edits ~f:(fun edit -> edit.column <> new_edit.column)
      in
...
let add_focused_sort_col (m : Model.t) =
  ...
  | Some col_id ->
      let table =
        Ts_table.Model.cycle_sorting m.table col_id
          ~next_dir:Ts_table.Sort_dir.next ~keep_existing_cols:()
      in
      { m with table }
...
let add_row (m : Model.t) =
  {
    m with
    rows = Map.set m.rows ~key:(Row_id.create ()) ~data:(Row.random_row ());
  }
...
let remove_row (m : Model.t) =
  ...
  | Some row_id ->
      ...
      let table =
        if
          Option.equal Row_id.equal (Some row_id)
            (Ts_table.Model.focus_row m.table)
...
let escape table_apply_action (m : Model.t) =
  if Option.is_some m.help_text then { m with help_text = None }
  else
    match m.edit_state with
    | Not_editing ->
...
let apply_action table (m : Model.t Incr.t) =
  ...
  fun (action : Action.t) state ~schedule_action ->
    let schedule_table_action action =
      schedule_action (Action.Table_action action)
    in
...
let update_visibility table (m : Model.t Incr.t) =
  let%map m = m and table = table in
  fun ~schedule_action ->
    let table =
      Component.update_visibility table
...
let key_handler ~inject =
    ...
    Keyboard_event_handler.of_command_list_exn
      [
        command
          ...
          ~description:"Move focus one row up" ~cond:is_not_text_input
          (fun _ev -> inject' (Ts_table.Action.move_focus_row Prev));
        command
          ~keys:[ key ArrowDown; key KeyJ ]
          ~description:"Move focus one row down" ~cond:is_not_text_input
          (fun _ev -> inject' (Ts_table.Action.move_focus_row Next));
        command
          ~keys:[ key ArrowLeft; key KeyH ]
          ~description:"Move focus one column left" ~cond:is_not_text_input
          (fun _ev -> inject' (Ts_table.Action.move_focus_col Prev));
        command
          ~keys:[ key ArrowRight; key KeyL ]
          ~description:"Move focus one column right" ~cond:is_not_text_input
          (fun _ev -> inject' (Ts_table.Action.move_focus_col Next));
        command
          ~keys:[ key PageUp ]
          ~description:"Move focus one page up" ~cond:is_not_text_input
          (fun _ev -> inject' (Ts_table.Action.page_focus_row Prev));
        command
          ~keys:[ key PageDown ]
          ~description:"Move focus one page down" ~cond:is_not_text_input
        ...
        command
          ~keys:[ key Enter ]
          ~description:"Commit edits" ~cond:is_not_search_input
          (fun _ev -> inject Action.Commit_edits);
        command
          ~keys:[ key ~shift:() KeyE ]
          ~description:"Start editing focused row" ~cond:is_not_text_input
          (fun _ev -> inject Action.Edit_start);
        command
          ~keys:[ key KeyS ]
          ~description:
            "Sort on focused column (in addition to existing sort columns)"
          ~cond:is_not_text_input
          (fun _ev -> inject Action.Add_focused_sort_col);
        command
          ~keys:[ key ~alt:() KeyA ]
          ~description:"Add a randomly-generated row" ~cond:is_not_text_input
          (fun _ev -> inject Action.Add_row);
        command
          ~keys:[ key ~alt:() KeyR ]
          ~description:"Remove a randomly chosen row" ~cond:is_not_text_input
  ...
  in
  let handler =
    Keyboard_event_handler.add_command_exn handler help_menu_command
  in
  let keydown_handler =
    Attr.on_keydown (fun ev ->
        Keyboard_event_handler.handle_or_ignore_event handler ev)
  in
...
let row_renderer (m : Model.t Incr.t) ~(inject : Action.t -> unit Vdom.Effect.t)
  ...
  fun ~row_id ~row ->
    let mode =
      ...
      let focused =
        [%compare.equal: Row_id.t option] (Some row_id) focused_row
      in
    ...
    let focus_nth_column col_num =
      inject
        (Action.Table_action (Ts_table.Action.set_focus_col (Some col_num)))
    in
    let%bind sort_columns = sort_columns in
    Row.view row ~mode ~sort_columns ~focused_column ~focus_me ~focus_nth_column
      ~remember_edit:(fun ~column value ->
        inject (Remember_edit { column; value }))

let view table (m : Model.t Incr.t) ~(inject : Action.t -> unit Vdom.Effect.t) =
  let open Vdom in
  let scroll_attr =
    Vdom.Attr.on_scroll (fun _ -> Vdom.Effect.Viewport_changed)
  in
  let key_handler, help_menu_command = key_handler ~inject in
  let help_text_view_spec =
    Help_text.View_spec.with_classes ~key_class:"help-text-key"
      ~plain_text_class:"help-text-plain-text"
  in
  let help_menu_hint =
    let help_text =
      Keyboard_event_handler.Command.get_help_text help_menu_command
    in
    Node.div
      ~attr:
        (Attr.style
           Css_gen.(combine (text_align `Center) (uniform_padding (`Px 5))))
  ...
  and maybe_help_menu =
    ...
    | Some help_text ->
        Node.div ~key:"help"
          ...
          [
            Node.div ~attr:(Attr.id "help-menu")
              [
  ...
  Node.div
    ...
    [
      maybe_help_menu;
      Node.div ~key:"top" ~attr:(Attr.id "top-container")
        [ input; help_menu_hint ];
      Node.div ~key:"table"
        ~attr:
          (Attr.many_without_merge [ Attr.id "table-container"; scroll_attr ])
        [ table ];
    ]
...
let should_set_edit_focus ~old_model (m : Model.t) =
  ...
  is_editing m
  && not
       ([%compare.equal: bool * Row_id.t option] (edit_state old_model)
          (edit_state m))
...
let set_edit_focus (m : Model.t) =
  ...
  | Editing _, Some _ -> (
      match
        Dom_html.getElementById_coerce "focus-on-edit" Dom_html.CoerceTo.input
      with
      | None -> ()
      | Some i -> i##select)
...
let on_display table ~old_model (m : Model.t Incr.t) =
  ...
  fun state ~schedule_action ->
    ...
    let editing (model : Model.t) =
      match model.edit_state with Not_editing -> false | Editing _ -> true
    in
    (* When the user presses the shift+e when the focus is out of view, scroll to it. *)
    if (not (editing old_model)) && editing m then
      ignore
        (Ts_table.Extra.scroll_focus_into_scroll_region m.table table_extra)
    else
...
let on_startup ~schedule_action (m : Model.t) =
  ...
  every (Time_ns.Span.of_ms 50.) (fun () ->
      schedule_action (Action.Big_kick ids));
  Deferred.unit
...
let create_table_component (model : Model.t Incr.t) ~old_model ~inject =
  ...
  Ts_table.create table_model ~old_model:old_table_model ~rows ~columns
    ~render_row
...
let init () : Model.t =
  let rows =
    Row.random_rows 100_000
    |> List.map ~f:(fun row -> (Row_id.create (), row))
    |> Row_id.Map.of_alist_exn
  in
  let table =
    Ts_table.Model.create ~scroll_margin:(Table.Margin.uniform 5.)
      ~scroll_region:(Element "table-container") ~float_header:Edge
      ~float_first_col:(Px_from_edge (-1)) ~height_guess ()
  in
